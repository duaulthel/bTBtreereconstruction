...
#------------------------------------
nnon_se <- c("A1", "B1", "B2", "S1") # do not change
...
for (j in c(1:2,4:6,8:9,11:16,18:19,22:29)){ #change to list of trees that converged
  ...
  for (i in 1:nb_scheme){ #sampling schemes
    ...
    #remove index case in order to consider transmission events
    data_sic <- data %>% filter(infector_id!=0) 
  ...
  if(j > 1){ #if this is not the first tree, call old file
    prev <- read_csv(paste0("transphylo_",samp,"_reff.csv"))
    nb <- rbind(prev, nb)
    write_csv(nb, paste0("transphylo_",samp,"_reff.csv"))
  }
